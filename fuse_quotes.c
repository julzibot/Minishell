...
/*   Updated: 2023/01/25 18:55:57 by mstojilj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*join_tokens(char *token, char **lex_tab, t_cmd *plist, int j)
{
	...
	if (plist->quoted[j + 1] && lex_tab[j + 1][len] \
			!= lex_tab[j][ft_strlen(lex_tab[j]) - 1])
		str = rem_quotes(str, 0);
	str = rem_quotes(str, 0);
	token = rem_quotes(token, 1);
	token = ft_strjoin(token, str, 3);
	return (token);
}
...
char	*fuse_quotes(char *token, char **lex_tab, t_cmd *plist, int j)
{
	...
	&& lex_tab[j + 1] && lex_tab[j + 1][0] == token[ft_strlen(token) - 1])
		join_next = 1;
	while (join_next && lex_tab[j + 1] && !plist->space_after[j])
	{
		type = token_type(lex_tab[j + 1], plist->quoted[j + 1]);
		token = join_tokens(token, lex_tab, plist, j);
		if (!(is_delim(token[ft_strlen(token) - 1]) == 1 \
		&& lex_tab[++j + 1] && lex_tab[j + 1][0] \
...
int	quotes_skip(char **tab, int *space_after)
{
	...
	count = 0;
	while (!space_after[count] \
		&& ((tab[0][ft_strlen(tab[0]) - 1] == '\"' \
		&& tab[1] && tab[1][0] == '\"') \
		|| (tab[0][ft_strlen(tab[0]) - 1] == '\'' \
		&& tab[1] && tab[1][0] == '\'')))
	{
